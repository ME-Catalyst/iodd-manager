...
Monitor IODD Manager in production for reliability and performance.
...
http_requests_total = Counter(
    'iodd_manager_http_requests_total',
    'Total HTTP requests',
    ['method', 'endpoint', 'status']
)

http_request_duration = Histogram(
    'iodd_manager_http_request_duration_seconds',
...
iodd_imports_total = Counter(
    'iodd_manager_imports_total',
    'Total IODD imports',
    ['status']  # success, failed
)

devices_total = Gauge(
    'iodd_manager_devices_total',
    'Total devices in database'
)

adapters_generated_total = Counter(
    'iodd_manager_adapters_generated_total',
...
```promql
rate(iodd_manager_http_requests_total[5m])
```

2. **Request Duration (P95)**
```promql
histogram_quantile(0.95, rate(iodd_manager_http_request_duration_seconds_bucket[5m]))
```
...
```promql
rate(iodd_manager_imports_total{status="success"}[5m]) /
rate(iodd_manager_imports_total[5m]) * 100
```

5. **Adapters Generated by Platform**
```promql
rate(iodd_manager_adapters_generated_total[5m])
```
  ...
  - job_name: iodd-manager
    static_configs:
      - targets:
        ...
        labels:
          job: iodd-manager
          __path__: /opt/iodd-manager/data/logs/*.log
```
...
```yaml
groups:
  - name: iodd_manager_alerts
    interval: 30s
    rules:
      # Service down
      - alert: ServiceDown
        expr: up{job="iodd-manager"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "IODD Manager is down"
          description: "IODD Manager has been down for more than 1 minute"

      # High error rate
      - alert: HighErrorRate
        expr: rate(iodd_manager_http_requests_total{status=~"5.."}[5m]) > 0.05
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: ImportFailures
        expr: rate(iodd_manager_imports_total{status="failed"}[5m]) > 0.1
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      # Slow requests
      - alert: SlowRequests
        expr: histogram_quantile(0.95, rate(iodd_manager_http_request_duration_seconds_bucket[5m])) > 2
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      # Disk space low
      - alert: DiskSpaceLow
        expr: (node_filesystem_avail_bytes{mountpoint="/opt/iodd-manager"} / node_filesystem_size_bytes) < 0.1
        for: 5m
        labels:
          severity: warning
        annotations:
...
*/5 * * * * /opt/iodd-manager/scripts/check-health.sh
```
...
### IODD Manager Dashboard

```json
{
  "dashboard": {
    "title": "IODD Manager Overview",
    "panels": [
      {
        "title": "Request Rate",
        "targets": [{
          "expr": "rate(iodd_manager_http_requests_total[5m])"
        }]
      },
      {
        "title": "Response Time P95",
        "targets": [{
          "expr": "histogram_quantile(0.95, rate(iodd_manager_http_request_duration_seconds_bucket[5m]))"
        }]
      },
      {
        "title": "Total Devices",
        "targets": [{
          "expr": "iodd_manager_devices_total"
        }]
      },
      {
        "title": "Import Success Rate",
        "targets": [{
          "expr": "rate(iodd_manager_imports_total{status='success'}[5m]) / rate(iodd_manager_imports_total[5m]) * 100"
        }]
      }
    ]
  }
